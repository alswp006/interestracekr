## Task: App 라우팅 연결 + Provider 조립(2개 라우트)
react-router-dom 라우팅을 고정하고 PolicyBlockProvider를 앱 최상단에 조립한다. 라우트는 정확히 2개(`/`, `*`)만 두고, `*`는 NotFoundRedirect로 연결한다. ErrorBoundary로 전체 라우트를 감싸 크래시를 UI로 흡수한다. main.tsx는 수정하지 않는다.
...
4. Read `.ai-factory/spec.md` for full SPEC context — 통합 패킷이므로 전체 앱 구조 파악 필요
...
- Packet 0011: "페이지: PolicyBlockHome(`/`)" (files: src/pages/PolicyBlockHome.tsx)
- Packet 0012: "페이지: NotFoundRedirect(`*`)" (files: src/pages/NotFoundRedirect.tsx)
- Packet 0014: "ErrorBoundary(흰 화면 방지)" (files: src/lib/ErrorBoundary.tsx)
