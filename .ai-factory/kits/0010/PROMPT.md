## Task: UI 컴포넌트: 배너 영역(AdSlotSection)
정책 안내 본문과 버튼 영역 사이에 배너를 비침투적으로 배치하기 위한 래퍼 컴포넌트를 만든다. 템플릿 제공 AdSlot을 그대로 사용하고, 상하 간격은 Spacing으로만 제어한다. 광고 SDK 직접 호출/추가 구현은 하지 않는다.
...
src/components/AdSlotSection.tsx
