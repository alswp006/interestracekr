## Task: 데이터 모델 타입 + RouteState 계약 정의
SPEC에 정의된 PolicyBlockAcknowledgement, LocalStorageHealth를 리터럴 타입까지 100% 동일하게 타입으로만 선언하고, 라우팅에서 location.state 캐스팅에 쓰는 RouteState 계약을 고정해 이후 모든 레이어가 이 타입만 import하도록 만든다.
