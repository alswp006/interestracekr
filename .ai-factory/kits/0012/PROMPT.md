## Task: 페이지: NotFoundRedirect(`*`)
차단 상태에서 어떤 경로로 들어와도 정책 안내(`/`)로 통일하기 위해 catch-all 페이지를 만든다. 이 페이지는 UI를 거의 가지지 않고, react-router-dom Navigate로 즉시 `/`로 리다이렉트한다. location.state는 RouteState 계약상 unknown으로만 취급한다.
