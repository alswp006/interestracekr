## Task: PolicyBlock 상태 관리(Context + Hook)
PolicyBlockHome 화면이 필요한 로딩/ack 상태/저장소 오류 노출/액션(확인 저장, 초기화)을 제공하는 Context+Hook을 만든다. 첫 페인트에서 health를 동기적으로 읽어 showStorageError 초기값을 결정해 AC-7(즉시 오류 문구) 요구를 만족시킨다.
...
src/lib/state/policyBlockStore.tsx
...
- Packet 0003: "PolicyBlockAcknowledgement 스토리지 CRUD" (files: src/lib/storage/policyAckStorage.ts)
- Packet 0004: "LocalStorageHealth 스토리지 CRUD + 무한 루프 방지" (files: src/lib/storage/localStorageHealthStorage.ts)
