## Task: UI 컴포넌트: 정책 상세 AlertDialog
"자세히 보기" 탭으로 열리는 AlertDialog를 컴포넌트로 분리한다. 제목/본문에 SPEC 고정 문구를 포함하고, 닫기 버튼 라벨은 반드시 "닫기"로 한다(취소 금지). 열림/닫힘은 open prop과 onClose 콜백으로 제어한다.
...
src/components/PolicyViolationDialog.tsx
