## Task: UI 컴포넌트: 로컬 데이터 초기화 BottomSheet
"로컬 데이터 초기화" 진입 시 표시되는 BottomSheet를 컴포넌트로 분리한다. "초기화하기"(danger)와 "닫기"(weak) 버튼을 제공하고, 초기화 성공/실패 후 닫힘은 부모가 제어한다. 내부에서 TextField/외부 링크를 추가하지 않는다.
...
src/components/ResetAckSheet.tsx
