## Task: UI 컴포넌트: 저장소 오류 문구(Notice)
showStorageError가 true일 때 PolicyBlockHome에서 노출할 고정 오류 문구 영역을 컴포넌트로 분리한다. 문구는 SPEC 고정 문자열을 그대로 사용하고, TDS Paragraph.Text + Spacing만 사용해 화면 하단에 자연스럽게 이어지게 한다.
...
src/components/StorageErrorNotice.tsx
