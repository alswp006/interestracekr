## Task: UI 유틸: 하단 Safe Area 패딩 컴포넌트
모바일(iOS 홈 인디케이터 등)에서 하단 버튼/토스트가 잘리지 않도록 safe-area-inset-bottom을 반영하는 레이아웃 컴포넌트를 제공한다. TDS 여백을 덮어쓰지 않고, 고정 영역이 아니라면 최소한의 paddingBottom만 적용한다.
...
src/components/BottomSafeAreaPadding.tsx
