## Task: 프로덕션 품질 가드(console 출력 억제)
SPEC의 "프로덕션 빌드에서 console.error 0건" 요구를 지키기 위해, production 환경에서 console.error를 no-op으로 바꾸는 가드를 제공한다. 개발 환경에서는 동작을 바꾸지 않으며, 앱 시작 시 1회만 호출되도록 설계한다.
