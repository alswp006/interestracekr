## Task: PolicyBlockAcknowledgement 스토리지 CRUD
policyBlock.ack.v1의 읽기/파싱/저장/삭제를 담당하는 storage 헬퍼를 구현한다. 파싱 실패를 LS_PARSE_ERROR로 반환해 상위가 손상 데이터로 처리할 수 있게 하고, 저장 시 createdAt 유지/재생성 규칙과 ISO 시간(new Date().toISOString())만 사용하는 규칙을 정확히 지킨다.
...
src/lib/storage/policyAckStorage.ts
