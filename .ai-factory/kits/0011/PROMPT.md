## Task: 페이지: PolicyBlockHome(`/`)
정책 위반 안내/차단 랜딩 화면을 SPEC Blueprint 그대로 구성한다. 로딩 문구, 상세 AlertDialog, 확인 저장(토스트 2초), 이미 확인 상태 표시, 저장소 오류 문구, 로컬 데이터 초기화 BottomSheet, 배너 AdSlot을 포함한다. 회원가입/로그인 UI/문구/TextField는 절대 추가하지 않는다.
...
- Description: 앱 진입 시 PRD에 명시된 오픈 정책 위반 사유를 사용자에게 고지하고, 
...
- Packet 0001: "데이터 모델 타입 + RouteState 계약 정의" (files: src/lib/types.ts)
- Packet 0005: "PolicyBlock 상태 관리(Context + Hook)" (files: src/lib/state/policyBlockStore.tsx)
- Packet 0006: "UI 컴포넌트: 차단 상태 패널(StatusPanel)" (files: src/components/PolicyBlockStatusPanel.tsx)
- Packet 0007: "UI 컴포넌트: 저장소 오류 문구(Notice)" (files: src/components/StorageErrorNotice.tsx)
- Packet 0008: "UI 컴포넌트: 정책 상세 AlertDialog" (files: src/components/PolicyViolationDialog.tsx)
- Packet 0009: "UI 컴포넌트: 로컬 데이터 초기화 BottomSheet" (files: src/components/ResetAckSheet.tsx)
- Packet 0010: "UI 컴포넌트: 배너 영역(AdSlotSection)" (files: src/components/AdSlotSection.tsx)
