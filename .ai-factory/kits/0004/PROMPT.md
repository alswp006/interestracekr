## Task: LocalStorageHealth 스토리지 CRUD + 무한 루프 방지
app.localStorageHealth.v1의 read/write를 구현한다. 핵심은 health 저장 자체가 Quota로 실패해도 예외를 흡수하고 재시도를 절대 하지 않도록(함수 1회 호출당 setItem 1회) 보장하는 것이다. createdAt 유지 규칙과 checkedAt/updatedAt 갱신을 ISO로 고정한다.
...
src/lib/storage/localStorageHealthStorage.ts
...
- Packet 0002: "localStorage 안전 래퍼(safeLocalStorage)" (files: src/lib/storage/safeLocalStorage.ts)
