## Task: 외부 이동 차단 런타임 가드
검수 반려 방지를 위해 앱 런타임에서 외부 링크 클릭/폼 액션 등을 막는 가드를 제공한다. window.open/window.location.href를 호출하지 않고, document 클릭 이벤트를 캡처해 http(s) 외부 href의 기본 동작을 preventDefault 처리한다.
