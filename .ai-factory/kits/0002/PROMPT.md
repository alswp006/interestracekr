## Task: localStorage 안전 래퍼(safeLocalStorage)
SecurityError 등 localStorage 접근 자체가 throw 가능한 환경을 대비해 절대 throw하지 않는 safeGetItem/safeSetItem/safeRemoveItem 래퍼를 만든다. QuotaExceededError를 감지해 에러 코드를 분기하며, src 내 console.error 추가 없이 에러를 값으로만 반환한다.
...
src/lib/storage/safeLocalStorage.ts
