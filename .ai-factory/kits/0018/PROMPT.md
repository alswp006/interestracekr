## Task: 최종 폴리시: 외부 이동 가드 적용 + 프로덕션 가드 + 정적 점검 스크립트
앱 시작 시 외부 이동 차단 가드와 프로덕션 품질 가드를 1회 초기화하도록 App.tsx를 폴리시한다. 또한 scripts/static-check.mjs와 package.json 스크립트를 추가해 금지 토큰(외부 이동/로그인 SDK/금지 문구 등)을 정적 점검한다. 라우팅 변경은 하지 않는다.
...
src/App.tsx, scripts/static-check.mjs, package.json
...
- Packet 0013: "외부 이동 차단 런타임 가드" (files: src/lib/externalNavigation.ts)
- Packet 0015: "프로덕션 품질 가드(console 출력 억제)" (files: src/lib/productionGuards.ts)
- Packet 0017: "App 라우팅 연결 + Provider 조립(2개 라우트)" (files: src/App.tsx)
