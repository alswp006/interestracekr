## Task: UI 컴포넌트: 차단 상태 패널(StatusPanel)
PolicyBlockHome 상단의 회색 안내 패널을 재사용 컴포넌트로 분리한다. 핵심 문구와 hasAcknowledged 상태에 따른 "이미 확인했어요" 표시(Chip)를 포함하고, TDS Spacing만으로 간격을 조절하며 HEX 색상 하드코딩 없이 TDS 토큰(var(--tds-color-* ))만 사용한다.
...
src/components/PolicyBlockStatusPanel.tsx
